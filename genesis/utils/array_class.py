...
def iter_data(struct, name: str, kind: DataKind | None = None) -> Iterator[DataItem]:
    ...
    struct-valued field applies to every array of that struct. A static config yields one item of kind CONFIG, and
    anything else raises and names the offending slot.
    """
...
def fill_data(items: Iterable[DataItem], values: Mapping[str, np.ndarray | torch.Tensor]) -> None:
    """Write into each array the value recorded under its name, rejecting a record made for another scene.
