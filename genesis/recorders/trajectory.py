...
A trajectory file holds the scene as 'Scene.export' writes it, then its frames in compressed chunks, then the final
state whole. Every part carries its own length and checksum, so a run that crashes leaves every chunk written before
readable, and a reader stops at the first part it cannot trust. A checkpoint file (see 'Scene.save_checkpoint') is a
trajectory file holding the final state alone.
"""
...
# A terminal record: layout length, data length, checksum of both.
LAST_HEADER = struct.Struct("<QQI")
LENGTH = struct.Struct("<Q")
# What a frame holds in each mode (see TrajectoryFile). The terminal record holds every array but the static configs.
COMPRESSED_KINDS = frozenset((DataKind.INFO, DataKind.STATE))
EXACT_KINDS = CHECKPOINT_KINDS
LAST_KINDS = frozenset(DataKind) - {DataKind.CONFIG, DataKind.CONSTANT}
...
def _read_frame(sim: "Simulator", kinds: frozenset[DataKind]) -> np.ndarray:
    ...
    if gs.use_zerocopy:
        parts = [
            qd_to_torch(value, copy=False).contiguous().reshape(-1).view(torch.uint8) for _, value, _ in sim.data(kinds)
        ]
...
def _header(source: "TrajectorySource", exact: bool, chunk_size: int, fields: list[FrameField]) -> bytes:
    """Return the magic, the JSON manifest and the scene description that open a trajectory file."""
...
def _last_record(sim: "Simulator") -> bytes:
    """Return the terminal record: the whole state of the simulation, every array but the static configs and constants,
    scratch included, since a debugging session inspects the final state closely.
    """
    layout = json.dumps(_fields_to_json(_frame_fields(sim, LAST_KINDS))).encode()
    data = zlib.compress(_read_frame(sim, LAST_KINDS).tobytes())
    return LAST_MAGIC + LAST_HEADER.pack(len(layout), len(data), zlib.crc32(data, zlib.crc32(layout))) + layout + data


def save_checkpoint(path: str | os.PathLike, source: "TrajectorySource") -> None:
    """Write the whole state of a built scene as a trajectory file without frames (see 'Scene.save_checkpoint')."""
    with open(path, "wb") as file:
        file.write(_header(source, exact=True, chunk_size=0, fields=[]))
        file.write(_last_record(source.sim))
...
class TrajectoryFileWriter(BaseFileWriter):
    ...
    for zlib to remove, and an unchanged info array costs nothing. Each chunk flushes to disk at once: memory use is
    one open chunk plus the queue, whatever the file size.
    """
    ...
    def _initialize_writer(self):
        ...
        self._header = _header(self._source, self._is_exact, self._options.chunk_size, self._fields)
        self._written = 0

    def read_frame(self) -> None:
        """Capture the frame of the coming step.

        The scene calls it before each step (see 'Scene.start_recording'), since 'Simulator.step' zeroes the applied
        forces the frame must hold. The recording pass after the step processes the kept frame.
        """
        self._frame = _read_frame(self._source.sim, self._kinds)
    ...
    def _write(self, record: bytes) -> bool:
        """Append one record, with the header ahead of the first, if the file may still grow by that much. Otherwise
        close the file and record the fault.
        """
    ...
    def _write_chunk(self):
        if self._n_chunk_frames == 0 or self._file is None:
            return
        payload = zlib.compress(bytes(self._chunk))
        header = CHUNK_MAGIC + CHUNK_HEADER.pack(self._n_chunk_frames, len(payload), zlib.crc32(payload))
        if self._write(header + payload):
            self._n_frames += self._n_chunk_frames
        self._chunk.clear()
        self._n_chunk_frames = 0
        self._previous = None
    ...
    def cleanup(self):
        ...
        if self._file is not None:
            # The terminal record follows the last chunk, so an error in its array reads costs only this record.
            if self._write(_last_record(self._source.sim)):
                self._file.close()
                self._file = None
...
class Trajectory:
    ...
    Frame i is the state the scene stood in before its step i. When the file holds the final state, it stands as one
    more frame after the last recorded one, so 'len' counts it and 'seek' reaches it.
    ...
    def __init__(
        ...
        from genesis.engine.scene import EnvironmentLayout, SceneCheckpoint
        ...
        with open(path, "rb") as file:
            ...
            # Each chunk is trusted by its checksum, so the offsets stop at the first record a crash left incomplete.
            self._chunk_offsets: list[int] = []
            self._chunk_ends: list[int] = []
            self._last_fields: list[FrameField] | None = None
            ...
            while True:
                ...
                if magic == CHUNK_MAGIC:
                    ...
                    if len(header) < LAST_HEADER.size:
                        break
                    layout_length, length, crc = LAST_HEADER.unpack(header)
                    layout = file.read(layout_length)
                    data = file.read(length)
                    if len(layout) < layout_length or len(data) < length or zlib.crc32(data, zlib.crc32(layout)) != crc:
                        break
                    self._last_fields = _fields_from_json(json.loads(layout))
                    self._last = np.frombuffer(bytearray(zlib.decompress(data)), np.uint8)
                    break
                else:
                    break
        if n_frames == 0 and self._last is None:
    ...
    def __len__(self) -> int:
        return self._n_frames + (self._last is not None)
    ...
    def frame(self, index: int, kinds: frozenset[DataKind] = frozenset(DataKind)) -> dict[str, np.ndarray]:
        """Return the arrays of frame 'index' of the given kinds by name, shaped as the scene holds them."""
        if not 0 <= index < len(self):
            gs.raise_exception(f"Frame {index} is out of the {len(self)} frames of '{self._path}'.")
        if index == self._n_frames:
            fields, frame = self._last_fields, self._last
        else:
            # The chunk holding the frame is decoded whole, undoing its chain of differences, and kept for the frames
            # of the same chunk read next.
            ...
            fields, frame = self._fields, self._cached_frames[index - i_frame_start]
        return {
    ...
    def seek(self, index: int) -> None:
        """Put the scene in the state of frame 'index'.

        An exact frame puts back everything a step reads or writes, so the scene steps on from there bit-for-bit. A
        compressed frame puts back the state and recomputes the link and geom poses by forward kinematics. The final
        state puts back what an exact frame does: the scratch it also holds is for inspection, and its shape follows the
        backend of the recording.
        """
        kinds = COMPRESSED_KINDS if index < self._n_frames and not self.is_exact else EXACT_KINDS
        ...
        self._scene.__setstate__(self._checkpoint_class(scene=self._scene.desc, layout=self._layout, sim=sim))

    def play(self, loop: bool = False) -> None:
        """Seek every frame in turn and redraw the scene at each, which plays the run in the viewer.

        With 'loop', the run starts over until the interactive viewer is closed, so the scene must have one.
        """
        if loop and self._scene.viewer is None:
            gs.raise_exception("Looping a trajectory needs an interactive viewer.")
        while True:
            for index in range(len(self)):
                self.seek(index)
                self._scene.visualizer.update(force=True)
            if not loop or not self._scene.viewer.is_alive():
                return
