...
def mpr_find_pos(
    ...
    # them: weighting the portal's three vertices equally puts the contact at their centroid, which lies on the portal
    # and keeps the ratio finite. Nothing finer is available - the weights are what carries where on the portal the
    # contact sits.
    if sum_ <= collider_info.mpr.CCD_EPS[None] * qd.abs(b).sum():
        b[0] = 0.0
        for i in qd.static(range(1, 4)):
