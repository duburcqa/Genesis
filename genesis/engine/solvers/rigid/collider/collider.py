...
class Collider:
    def __init__(self, rigid_solver: "RigidSolver"):
        ...
        # portal-dependent: only an EXACT portal recovers the true depth (Thm 4.2) and so stays on MPR to deeper
        # penetrations (valid ratio); every other portal bounds the depth at best and falls back sooner (base ratio).
        # The valid ratio is capped below the point where trusting deep exact portals lets contacts pump energy.
        ...
        # having resolved a deeper, non-minimal portal (then upgraded to GJK). At the gate this jump only tightens the
        # overlap threshold, clamped into [tolerance, overlap_ratio * geom_pair_scale], so a cold pair (cached
        # penetration reset to 0) is judged on the overlap alone and never on the tolerance.
    ...
    def _init_static_config(self) -> None:
        ...
        # the island construction - reads through. It runs on every backend: the physical layout is racy on GPU, where
        # the narrowphase reserves slots via atomic_add, but even a serial narrowphase enumerates pairs in broadphase
        # sweep order, which is taken along a fixed world axis and so reinterleaves as soon as the scene turns. A
        # rigidly rotated copy of a scene would otherwise hand the solver the contacts of a pair in a different order.
        # Disabled only in autodiff mode: get_contacts applies the permutation but func_set_upstream_grad writes
        # upstream gradients back by physical index, so a non-identity permutation would attach gradients to the wrong
        # contacts.
