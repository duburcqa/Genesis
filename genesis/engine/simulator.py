...
class Simulator(RBC):
    ...
    def __setstate__(self, state: SimulatorCheckpoint) -> None:
        ...
        """
        solvers = {type(solver).__name__: solver for solver in self._active_solvers}
        if set(state.solvers) != set(solvers):
            gs.raise_exception(
                f"The checkpoint holds {sorted(state.solvers)} where this scene simulates {sorted(solvers)}."
            )
        for name, solver in solvers.items():
            solver.__setstate__(state.solvers[name])
