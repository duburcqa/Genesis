...
- In the title of your Pull Request, please include [BUG FIX], [FEATURE] or [MISC] to indicate the purpose, preceded by [CHANGING] (the simulated physics change by default: a different model, or different default parameters) or [BREAKING] (API break) when it applies, e.g. [CHANGING][MISC]. The title is a sentence and ends with a period.
