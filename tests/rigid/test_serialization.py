...
@pytest.fixture
def checkpoint_scene(mimic_hinges, show_viewer):
    scene = gs.Scene(
        viewer_options=gs.options.ViewerOptions(
...
@pytest.mark.required
@pytest.mark.parametrize("n_envs", [0, 2])
def test_pickle_resume(n_envs, checkpoint_scene, tmp_path, show_viewer):
    scene = checkpoint_scene
...
def test_trajectory_replay(n_envs, checkpoint_scene, tmp_path, show_viewer, caplog, tol):
    ...
    # The final state closes each file, as one more frame. Both trajectories play in the same scene.
    ...
    # The final state is the frame after the last recorded one, and seeking it puts the scene where the run ended
    assert_equal(
